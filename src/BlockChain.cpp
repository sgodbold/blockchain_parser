...
#define MAX_BITCOIN_ADDRESSES 50000000 // 40 million unique addresses.
...
public:
	...
	void zombieReport(uint32_t zdays,float minBalance)
	{
		...
		time_t currentTime;
		time(&currentTime); // get the current time.
		logMessage("Scanning %s bitcoin addresses and looking for zombies.\r\n", formatNumber(mAddresses.size()));
...
public:
	...
	virtual void zombieReport(uint32_t zdays,float minBalance)
	{
